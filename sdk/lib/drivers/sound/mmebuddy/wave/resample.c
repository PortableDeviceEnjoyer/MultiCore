    ...
    _Out_ PVOID* OutputBuffer)
{
    ...
    ULONG CurrentBitsPerSample, CurrentChannels, CurrentSampleRate;

    /* Get input data */
    BufferOut = InputBuffer;
    BufferLength = InputLength;
    CurrentBitsPerSample = WaveFormatEx->wBitsPerSample;
    CurrentChannels = WaveFormatEx->nChannels;
    CurrentSampleRate = WaveFormatEx->nSamplesPerSec;

    /* Do the resampling */
    if (CurrentBitsPerSample != DataRange->MaximumBitsPerSample)
    {
        ...
        CurrentBitsPerSample = DataRange->MaximumBitsPerSample;
    }

    if (CurrentChannels != DataRange->MaximumChannels)
    {
        ...
        CurrentChannels = DataRange->MaximumChannels;
    }

    if (CurrentSampleRate > DataRange->MaximumSampleFrequency)
    {
        ...
        BufferOut = BufferOutTemp;
        BufferLength = BufferLengthTemp;
        CurrentSampleRate = DataRange->MaximumSampleFrequency;
    }
